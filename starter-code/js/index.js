...
window.onload = function(){
  ...
  createItemButton.onclick = function() {
    ...
    bodySel.insertBefore(newNode, newItemSel);
    ...
    // <div class="item">
    //   <div><span>IronShirt</span></div>
    //   <div>$<span class="item-price">15.00</span></div>
    //   <div><label for="quantity">QTY</label><input type="text" placeholder="0"/></div>
    //   <div>$<span class="total-item-price">0.00</span></div>
    //   <div><button class="btn-delete">Delete</button></div>
    // </div>
  ...
  function deleteItem(item){
    bodySel.removeChild(itemsSel[item]);
  };
  ...
  calculatePriceButton.onclick = function () {
    ...
    // OPTIONAL FOR LOOP
    // for (var i=0; i<itemCostSel.length; i++) {
    //   // get cost of one item
    //   var itemCost = itemCostSel[i].innerHTML;
    //   // get quantity of one item    
    //   var quantity = quantitySel[i].value;
    //   if (quantity >= 0) {
    //     var totalItemCost = itemCost*quantity;
    //   } else {
    //     var totalItemCost = 0;
    //   };      
    //   totalItemCostSel[i].innerHTML = totalItemCost;
    // };
