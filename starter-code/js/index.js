...
function getPriceByProduct(itemNode){
  ...
  var unityPrice = itemUnityPrice.innerHTML.substring(1);
  var itemQuantity = itemInfo[2].getElementsByClassName('quantity')[0].value;

  return parseFloat(unityPrice) * parseFloat(itemQuantity);
}

function updatePriceByProduct(productPrice, index){
  var actualPrice = document.getElementsByClassName('item-subtotal')[index];
  actualPrice.innerHTML = "$" + productPrice.toFixed(2);
}
...
function getTotalPrice() {
  ...
  actualTotalPrice.innerHTML = "$" + totalPrice.toFixed(2);
}
